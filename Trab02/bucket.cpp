...
bool loadBucket(int bucketId, Bucket *bucket) {
  ...
  // Lê as entradas
  for (int i = 0; i < BUCKET_CAPACITY; i++) {
    file.read(reinterpret_cast<char *>(&bucket->entries[i].linhaNum),
              sizeof(int));
  }
...
bool saveBucket(int bucketId, const Bucket *bucket) {
  ...
  if (!file.is_open()) {
    std::cerr << "Erro: nao foi possivel salvar o bucket " << bucketId
              << std::endl;
    return false;
  }
  ...
  // Escreve as entradas
  for (int i = 0; i < BUCKET_CAPACITY; i++) {
    file.write(reinterpret_cast<const char *>(&bucket->entries[i].linhaNum),
               sizeof(int));
  }
